...
if (gamePath == null || outputPath == null)
{
    ...
    Console.WriteLine("  assetextractor <game-path> <output-path> [--filter <pattern>] [--ui-only]");
    Console.WriteLine();
    ...
    Console.WriteLine("  --ui-only    Extract only interface assets (basename starts with i_).");
    Console.WriteLine();
    ...
    Console.WriteLine(@"  assetextractor ""C:\Program Files (x86)\NetDevil\Auto Assault"" assets/ui --ui-only");
    return 1;
}

return AssetExtractor.Run(gamePath: gamePath, outputPath: outputPath, filter: filter, uiOnly: uiOnly);
